import { SNIPPETS_DIR, generate } from "../../../snippets/util";
...
export default async function ({ message, args, api }: Context) {
  ...
  if (!title) return api.createMessage(message.channel_id, { content: "Missing title" });
  ...
  if (!code) return api.createMessage(message.channel_id, { content: "Missing code" });
  const author = message.author.username + "#" + message.author.discriminator;
  const file = generate({ title, code, author, description });

  const { id } = await api.createMessage(message.channel_id, { content: "Preparing..." });
  ...
  await exec("git add .", { cwd: SNIPPETS_DIR });
  await exec('git commit -m "Helperboat: Add snippet"', { cwd: SNIPPETS_DIR });
  await exec("git push", { cwd: SNIPPETS_DIR });
